...
export class MyRequestsComponent implements OnInit {
  ...
  public which_page =
    "my_requests"; /* option = {my_requests, my_requests_chat} */
  numCols;
  bookNavBarEnabled;
  bookSelected: Book;
  ...
  waiting_owner_reject_arr: Array<Book>;
  ...
  removeRequest() {
    ...
    if (
      this.bookSelected &&
      this.bookSelected.currentRequest &&
      this.bookSelected.currentRequest.approved
    ) {
      let dialogRef = this.dialog.open(DialogTwoButtonComponent, {
        width: "250px",
        data:
          "Request was already approved, Do you want to send cancel message to book owner?"
      });
      //check if user is sure he wish to remove this request
      dialogRef.afterClosed().subscribe(result => {
        if (result !== "cancel") {
          this.store.dispatch(new fromStore.MoveToWaitingReject(this.bookSelected));
          if (
            this.bookSelected &&
            this.bookSelected.ownerUid === this.selfUserId
          ) {
          ...
          let message: Message = {
            ...
            content:
              "Request was canceled by other user, reject request if book was returned."
          };
          this.store.dispatch(
  ...
  showBookNavbar(book: Book) {
    ...
    if (
      book.currentRequest &&
      !book.currentRequest.pending &&
      book.currentRequest.approved
    ) {
  ...
  initArrayStatus() {
    ...
    this.waiting_owner_reject_arr = new Array<Book>();

    for (let book of this.myRequests) {
      if(book.currentRequest.waitingReject){
        //status: waiting owner confirmation
        this.waiting_owner_reject_arr.push(book);
      }
      else if (book.currentRequest.pending) {
        //status: waiting approval
        this.waiting_approval_arr.push(book);
      } else if (
        book.currentRequest.approved &&
        book.currentRequest.hasNewMessages
      ) {
        //status: new message
  ...
  ngOnInit() {
    this.myRequestsSubscription = this.store
      .select(fromStore.getUserRequests)
      .subscribe(state => {
        this.myRequests = state;
    ...
    this.selfUserSubscribtion = this.store
    ...
    this.bookNavBarEnabled = false;
    this.whichPageSubscription = this.store
      .select<any>(fromStore.getContextmyRequestsOption)
      .subscribe(state => {
